// scripts/step9_complete_resume_workflow_REPROCESS.js - REPROCESS ALL CANDIDATES (Ignores Previous State)
// This script re-processes ALL resumes regardless of previous processing
// Use this when you want to regenerate drafts with updated critique/prompt
// It uses a SEPARATE state file to not interfere with original workflow
...
const args = minimist(process.argv.slice(2), {
  ...
  default: {
    ...
    // USE SEPARATE STATE FILE for reprocessing
    state: "resume_processing_state_REPROCESS.json",
    browser: "chromium",
    headless: false,
    headful: true,
    confirm: false,
    reset: false,
    slowMo: "150",
    max: 500  // Default to higher max for reprocessing (can handle 470+ conversations)
  }
});

args.max = Number(args.max) || 500;
...
async function sendResumeDownloadWebhook(clientName, resumeStatus, emailId, threadId) {
  try {
    ...
    const payload = {
      ...
      resumeStatus: resumeStatus || 'Failed',
      emailId: emailId || 'N/A',
      threadId: threadId || null,
      status: 'success',
      timestamp: new Date().toISOString(),
      reprocess: true  // Flag to indicate this is a re-processed draft
    };
...
// REPROCESS STATE - Does NOT check previous processed list
let RESUME_STATE = { 
  processed: [], 
  downloaded: [],
  readable: [],
  unreadable: [],
  reprocessed: []  // Track reprocessed candidates separately
};
...
function loadResumeState() {
  ...
  if (!RESUME_STATE.reprocessed) RESUME_STATE.reprocessed = [];
  
  log('📁 Loaded REPROCESS state file:', args.state);
  log('📊 Previously reprocessed:', RESUME_STATE.reprocessed.length, 'candidates');
}
...
function organizeResumeFile(fileInfo, isReadable) {
  ...
  log(`   📁 Category: ${category.toUpperCase()} (Google Drive only)`);
...
// MAIN WORKFLOW - REPROCESS VERSION (Ignores previous processed state)
async function processResumeWorkflow(page, fileInfo, clientName, threadId) {
  const result = {
    ...
    reprocessed: true,  // Flag indicating this is a re-process
    stages: {
      downloaded: true,
      readable: false,
      driveUploaded: false,
  ...
  try {
    ...
    // Update REPROCESS state (not the original processed array)
    if (readable) {
    ...
    log(`\n5️⃣  AI CRITIQUE GENERATION (UPDATED PROMPT)`);
    ...
    if (result.stages.draftCreated) {
      log(`✅ REPROCESS COMPLETE: ${clientName}`);
    } else {
      log(`⚠️  REPROCESS PARTIAL: ${clientName}`);
    }
    log(`${"=".repeat(60)}\n`);
...
(async () => {
  ...
  log('\n🚀 STEP 9 REPROCESS: Resume Workflow (Ignores Previous State)');
  log('='.repeat(60));
  log('🔄 REPROCESSING ALL CANDIDATES - Will create NEW drafts with updated critique');
  log('📁 Using separate state file:', args.state, '\n');
  
  if (args.reset) {
    log('⚠️  RESET MODE: Will process ALL conversations again\n');
    RESUME_STATE = { processed: [], downloaded: [], readable: [], unreadable: [], reprocessed: [] };
    saveResumeState();
  }
  ...
  try {
    ...
    for (const thread of threads) {
      ...
      // ✅ KEY DIFFERENCE: We do NOT check RESUME_STATE.processed
      // This allows re-processing ALL candidates regardless of previous state
      // Only skip if explicitly in reprocessed list (to avoid duplicates in same run)
      if (RESUME_STATE.reprocessed && RESUME_STATE.reprocessed.includes(name)) {
        log(`\n⏭️  Skipping ${name} (already reprocessed in this run)`);
        processed++;
        continue;
      }
      ...
      if (attachments.length > 0) {
        ...
        if (args.confirm) {
          ...
          if (downloadedFile) {
            ...
            // ✅ WEBHOOK 1: Log resume download (with reprocess flag)
            ...
            if (workflowResult.stages.readable) {
              ...
              if (workflowResult.stages.draftCreated) {
                ...
                // ✅ WEBHOOK 2: Log draft creation (with reprocess flag)
                if (isValidCandidateName(name)) {
                  await sendDraftCreatedWebhook(name, 'Success');
                }
                
                // ✅ SAVE DRAFT-LINKEDIN MAPPING (append to existing)
                try {
                  ...
                  // Add reprocess flag to distinguish from original drafts
                  const mappingRecord = {
                    ...
                    reprocessed: true,  // Flag indicating this is a re-processed draft
                    originalDraftId: null,  // Could track original if needed
                  ...
                  log(`   📝 Saved REPROCESS draft-LinkedIn mapping: ${name} -> draft ${workflowResult.draftInfo.draftId}`);
                } catch (mappingError) {
            ...
            // Track in reprocessed list (not processed to avoid conflict)
            if (!RESUME_STATE.reprocessed) RESUME_STATE.reprocessed = [];
            if (!RESUME_STATE.reprocessed.includes(name)) {
              RESUME_STATE.reprocessed.push(name);
            }
            
            // Also track in downloaded
